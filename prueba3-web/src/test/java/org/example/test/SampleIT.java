...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "prueba3-ear.ear");

        // Current directory is the root of the "prueba3-web" project. Go up one level
        // and enter the "prueba3-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../prueba3-ejb/target/prueba3-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "prueba3-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/prueba3-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "prueba3-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
